...
## Final Reflection
This assignment taught me that working with linked lists requires hyper-vigilance about pointer tracking, as one minor mistake can isolate portions of your data structures into memory leaks. Working with a Doubly Linked List was trickier because you have double the references (`prev` and `next`) to handle, but its ability to read backward makes tasks like checking for palindromes highly efficient without requiring extra data collections.vv